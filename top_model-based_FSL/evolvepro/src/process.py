...
def generate_single_aa_mutants_csv(wt_fasta, output_file, positions=None):
    """
    Generate a CSV file containing all possible single amino acid mutations.
    Args:
        wt_fasta (str): Path to the FASTA file containing the wild-type protein sequence.
    ...
    for i in positions_to_mutate:
        ...
        for mutant_aa in aa_alphabet:
            if mutant_aa != wt_aa:
                ...
                # Format variant name (e.g. Y3M)
                variant = f"{wt_aa}{i+1}{mutant_aa}"
                
                # Append to data list
                data.append({"variant": variant, "seq": mutant_sequence})
    
    # Write to CSV
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    with open(output_file, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["variant", "seq"])
        writer.writeheader()
        writer.writerows(data)
    
    # Print the number of mutants generated
    print(f"Number of mutants: {len(data)}")