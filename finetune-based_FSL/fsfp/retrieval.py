...
class Retriever():
    ...
    def get_save_path(self, prefix):
        args = self.args
        if prefix == 'vectorize':
            path =  f'{config.retr_dir}/vectors_{args.model}.pkl'
        else:
            path =  f'{config.retr_dir}/topk_{args.model}_{args.metric}.pkl'
        path = path.replace('.pkl', f'{args.save_postfix}.pkl')
        return path
    ...
    def __call__(self, proteins=None):
        ...
        if args.mode == 'vectorize':
            ...
            #创建一个字典，key为query的protein的名字，value由两部分：tgt_names为topk的目标蛋白质数据集名字，scores为对应的余弦相似度
